  ...
  public:
    static double yearFraction(date accrual_start, date accrual_end) {
      ...
      if(accrual_start.year() == accrual_end.year() ||
         gregorian_calendar::is_leap_year(accrual_start.year()) == gregorian_calendar::is_leap_year(accrual_end.year())) {
        double accural_days = date_duration(accrual_end - accrual_start).days();
        double days_in_year = gregorian_calendar::is_leap_year(accrual_start.year()) ? 366 : 365;
        ...
        double accural_days_p1 = date_duration(p1_end - accrual_start).days();
        double accural_days_p2 = date_duration(accrual_end - p1_end).days();

        double days_in_year_p1 = gregorian_calendar::is_leap_year(accrual_start.year()) ? 366 : 365;
        double days_in_year_p2 = gregorian_calendar::is_leap_year(accrual_end.year()) ? 366 : 365;
